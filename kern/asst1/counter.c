...
static struct lock* counter_lock;   // make global struct lock* so we can access everwhere
...
int counter_initialise(int val)
{
        ...
        counter_lock = lock_create("counter lock"); // initialise lock
        ...
        if (counter_lock == NULL) {
            return ENOMEM; // failed to init lock, return error no memory
        }
