...
/* Resource order:
   ...
   threads acquire locka THEN lockb if both are needed */
...
static void bill(void * unusedpointer, unsigned long unusedint)
{
        ...
        for (i = 0; i < NUM_LOOPS; i++) {
                ...
                lock_acquire(locka);    // bill acquires & releases in intended order
                lock_acquire(lockb);
...
static void ben(void * unusedpointer, unsigned long unusedint)
{
        ...
        for (i = 0; i < NUM_LOOPS; i++) {
                ...
                //lock_acquire(lockb);  <- original
                lock_acquire(locka);
                lock_acquire(lockb);    // moved lockb acquisition to after locka
                ...
                //lock_release(locka);  <- original
                lock_release(lockb);
                lock_release(locka);    // release locka last
        }
...
int twolocks (int data1, char ** data2)
{
        ...
        error = thread_fork("nob thread", NULL, &bob, NULL, 0); /* start
                                                                   Bob */
